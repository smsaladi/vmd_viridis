#!/usr/bin/env tclsh
##
##  Makes perceptually uniform and colorblind accessible color scales
##  available in VMD
##
##
...
##  public viewing. This script provides additional color scales to allow for
##  an alternative to the default rainbow coloring that is unambiguous both to
##  colorblind and non-colorblind people.
##
##  The default color scale is changed to Turbo and the following procedures
##  defined: `inferno_scale`, `magma_scale`, `plasma_scale`, `viridis_scale`,
##  `cividis_scale`, `turbo_scale`. Call the procedure that corresponds to the
##  scale you intend to use.
##
##  Color scale details:
...
##
##  Here each color scale is represented as 256 RGB values. These are then expanded to
##  1024 (as necessary for vmd) by linear interpolation. The following scripts from John
##  Stone are useful reference:
##  https://www.ks.uiuc.edu/Research/vmd/mailing_list/vmd-l/att-12719/colorscale.tcl
##  https://www.ks.uiuc.edu/Research/vmd/mailing_list/vmd-l/att-29771/matlab_colorscale.tcl
##
##  For further detail/future revisions, visit https://shyam.saladi.org/vmd_viridis
## 
##
##  Author: Shyam Saladi
##  Date: Nov 2019
##  License: MIT
##
##  Palette RGB values are taken from bokeh.palettes
##  Corresponding copyright notices can be found here:
##  https://github.com/bokeh/bokeh/blob/b19f2c5/bokeh/palettes.py
##
## 

# Linear interpolation (use _ avoid 'polluting' the namespace)
proc _lerpcolor { col1 col2 alpha } {
  set dc [vecsub $col2 $col1]
  set nc [vecadd $col1 [vecscale $dc $alpha]]
  return $nc
}

# Converts a hex color string `#NNNNNN` to fractional rgb values
proc _hextofrac { col } {
  scan $col "#%2x%2x%2x" r g b
  set r [expr $r / 255.]
  set g [expr $g / 255.]
  set b [expr $b / 255.]
  return [list $r $g $b]
...
proc set_color_scale { colors } {
  ...
  set n_cols [llength $colors]

  for {set i $mincolorid} {$i < $maxcolorid} {incr i} {
    set colpcnt [expr ($i - $mincolorid) / double($colrange)]

    set colidx [expr $colpcnt * $n_cols]
    set frac [expr $colidx - {floor($colidx)}]
    set colidx [expr {int($colidx)}]
    
    set colorA [_hextofrac [lindex $colors $colidx]]
    set nextidx [expr $colidx + 1]

    # Handle when we're at the last color of `colors`
    if {$nextidx < $n_cols} {
      set colorB [_hextofrac [lindex $colors $nextidx]]
    } else {
      set colorB $colorA
    }

    set colorC [_lerpcolor $colorA $colorB $frac]
    foreach {r g b} $colorC {}

    # For debugging/inspection:
    # puts "index: $i $r $g $b   -- $colpcnt $colidx $frac"
...
#
# Procedures to set scales and then use viridis as default
#

proc inferno_scale {} {
  variable new_color_scales
  set_color_scale [dict get $new_color_scales inferno]
}
proc magma_scale   {} {
  variable new_color_scales
  set_color_scale [dict get $new_color_scales magma]
}
proc plasma_scale  {} {
  variable new_color_scales
  set_color_scale [dict get $new_color_scales plasma]
}
proc viridis_scale {} {
  variable new_color_scales
  set_color_scale [dict get $new_color_scales viridis]
}
proc cividis_scale {} {
  variable new_color_scales
  set_color_scale [dict get $new_color_scales cividis]
}
proc turbo_scale   {} {
  variable new_color_scales
  set_color_scale [dict get $new_color_scales turbo]
}

puts "New scales are available via the following commands:
`inferno_scale`, `magma_scale`, `plasma_scale`, `viridis_scale`,
`cividis_scale`, & `turbo_scale`.

Viridis is loading now..."
